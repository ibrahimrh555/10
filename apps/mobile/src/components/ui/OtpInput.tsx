...
export const OtpInput = forwardRef<OtpInputHandle, Props>(function OtpInput({ value, onChange, error }, ref) {
  ...
  return <View accessibilityLabel="Code de vérification à six chiffres" accessibilityHint={error ? "Le code saisi contient une erreur" : "Saisissez les six chiffres reçus par e-mail"} onTouchEnd={() => input.current?.focus()} style={styles.row}>
    {Array.from({ length: OTP_LENGTH }, (_, index) => <View key={index} style={[styles.cell, value.length > index && styles.filled, value.length === index && styles.focused, error && styles.error]}>{value[index] ? <AppText variant="headingLg" style={styles.digit}>{value[index]}</AppText> : null}</View>)}
...
const styles = StyleSheet.create({
  row: { flexDirection: "row", justifyContent: "space-between", position: "relative", width: "100%" },
  cell: {
    alignItems: "center",
  ...
  filled: { borderColor: theme.colors.primary, borderWidth: 2 },
  focused: { borderColor: theme.colors.primary, borderWidth: 2 },
  error: { borderColor: theme.colors.danger },
  digit: { color: theme.colors.textPrimary, fontFamily: "sans-serif-condensed", fontSize: 28, fontWeight: "800", lineHeight: 32, textAlign: "center" },
  hidden: { bottom: 0, left: 0, opacity: 0.01, position: "absolute", right: 0, top: 0 },
});
