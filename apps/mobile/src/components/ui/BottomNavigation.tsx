...
export function BottomNavigation({ active = "Accueil" }: { active?: (typeof items)[number]["label"] | "Explorer" }) {
  return <View style={styles.bar}>{items.map((item) => { const selected = item.label === active || (active === "Explorer" && item.label === "Matchs"); return <Pressable accessibilityLabel={item.label} accessibilityRole="tab" accessibilityState={{ selected }} key={item.label} onPress={() => router.replace(item.route)} style={styles.item}><Ionicons name={item.icon} size={22} color={selected ? "#D7FF00" : "#FFFFFF"} /><AppText variant="caption" style={{ color: selected ? "#D7FF00" : "#FFFFFF", fontWeight: selected ? "700" : "500" }}>{item.label}</AppText></Pressable>; })}</View>;
}

const styles = StyleSheet.create({ bar: { backgroundColor: "#513737", borderColor: "#513737", borderRadius: 34, borderWidth: 1, flexDirection: "row", marginBottom: 6, marginHorizontal: 12, minHeight: 68, overflow: "hidden", paddingHorizontal: theme.spacing.xxs, paddingTop: 4 }, item: { alignItems: "center", flex: 1, gap: 2, justifyContent: "center", minHeight: 56 } });
