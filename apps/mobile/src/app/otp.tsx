...
export default function OtpScreen() {
  ...
  useEffect(() => { if (!pending.email || !pending.cityId) router.replace("/sign-in"); }, [pending.cityId, pending.email]);
  useEffect(() => { const timer = setInterval(() => setNow(Date.now()), 1000); return () => clearInterval(timer); }, []);
  const verify = useMutation({
    mutationFn: async () => {
      if (!pending.email || !pending.cityId) throw new Error("missing_pending_auth");
  ...
  const resend = useMutation({
    ...
    onSuccess() { if (!pending.email || !pending.cityId) return; pendingAuthStore.setPendingAuth(pending.email, pending.cityId, Date.now()); setNow(Date.now()); setCode(""); setError(null); setExpired(false); Alert.alert("Code envoyé", "Un nouveau code vient de vous être envoyé."); },
  ...
  if (!pending.email || !pending.cityId) return null;
  return <ScreenContainer footer={<AppButton disabled={!isOtpComplete(code) || expired || verify.isPending} loading={verify.isPending} label="Vérifier le code" icon="arrow-forward" onPress={() => verify.mutate()} />}>
    <AppHeader onBack={() => { pendingAuthStore.clearPendingAuth(); setCode(""); router.replace("/sign-in"); }} />
    ...
    <Pressable accessibilityRole="button" onPress={() => { setCode(""); router.replace("/sign-in"); }} style={styles.action}><AppText color="primary">Modifier l’adresse e-mail</AppText></Pressable>
  </ScreenContainer>;
}
