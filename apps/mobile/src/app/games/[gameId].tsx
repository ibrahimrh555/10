...
export default function GameDetailScreen() {
  ...
  const action = () => router.push({ pathname: "/games/confirm", params: { gameId: game.id } });
  return <SafeAreaView edges={["top", "left", "right"]} style={styles.safe}><View style={styles.page}>
    ...
    <View style={styles.fixed}><View style={styles.sticky}><Pressable accessibilityRole="button" onPress={action} style={({ pressed }) => [styles.button, pressed && styles.pressed]}><AppText style={styles.buttonText}>Rejoindre ce match</AppText></Pressable></View><BottomNavigation active="Matchs" /></View>
  </View></SafeAreaView>;
}
...
const styles = StyleSheet.create({
  safe: { backgroundColor: "#F7F6EF", flex: 1 }, page: { flex: 1 }, scroll: { paddingBottom: 190 },
  ...
  fixed: { backgroundColor: "#F7F6EF", bottom: 0, left: 0, position: "absolute", right: 0 }, sticky: { backgroundColor: "#FFFFFF", padding: 12 }, button: { alignItems: "center", backgroundColor: "#0A3828", borderRadius: 12, justifyContent: "center", minHeight: 53 }, buttonText: { color: "#FFFFFF", fontSize: 16, fontWeight: "700" }, pressed: { opacity: .82 },
