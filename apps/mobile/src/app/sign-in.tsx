...
const schema = z.object({ email: z.string().trim().toLowerCase().email("Saisissez une adresse e-mail valide."), cityId: z.string().min(1, "Choisissez une ville.") });
type FormValues = z.infer<typeof schema>;
type City = { id: string; name: string; countryCode: string };

export default function SignInScreen() {
  const pending = pendingAuthStore.getState();
  const { setValue, watch, setError, formState: { errors } } = useForm<FormValues>({ defaultValues: { email: pending.email ?? "", cityId: pending.cityId ?? "" } });
  ...
  const cities = useQuery({ queryKey: ["cities", debouncedSearch], queryFn: () => debouncedSearch ? trpcClient.cities.search.query({ search: debouncedSearch }) : trpcClient.cities.list.query() });
  const rows = (cities.data?.cities ?? []) as City[];
  const valid = useMemo(() => schema.safeParse({ email, cityId }).success, [email, cityId]);
  const sendOtp = useMutation({
    ...
    onSuccess(values) { pendingAuthStore.setPendingAuth(values.email, values.cityId, Date.now()); router.push("/otp"); },
  ...
  return <ScreenContainer footer={<AppButton disabled={!valid || sendOtp.isPending} loading={sendOtp.isPending} label="Recevoir le code" icon="arrow-forward" onPress={submit} />}>
    <AppHeader /><ScreenTitle dark="CONNEXION" />
